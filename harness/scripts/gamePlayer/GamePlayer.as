package gamePlayer
{
   ...
   public class GamePlayer extends EventDispatcher
   {
      ...
      public function harnessStart(media:AbstractMedia) : void
      {
         ...
         harnessFloor(0,440,960,60,SurfaceType.GROUND);
         harnessFloor(-30,0,30,500,SurfaceType.WALL);
         harnessFloor(960,0,30,500,SurfaceType.WALL);
         harnessFloor(300,385,100,55,SurfaceType.GROUND);
         harnessFloor(530,330,130,25,SurfaceType.GROUND);
         mcContainer.graphics.beginFill(0x30BD79,0.6);
         mcContainer.graphics.drawRect(850,340,65,100);
         mcContainer.graphics.endFill();
      ...
      private function harnessSpawn() : void
      {
         ...
         oPlayer = new PlayableCharacter(harnessMedia,new Point(100,290),false);
      ...
      private function harnessUpdate(event:UpdateEvent) : void
      {
         ...
         if(!bGameCompleted && p.x*30 > 850 && p.y*30 > 330)
         {
            bGameCompleted = true;
            oPlayer.setControlsEnabled(false);
            BuilderMain.instance.diagnosticLog("FINISH reached. Harness position check passed. Press R to restart.");
         }
      }
