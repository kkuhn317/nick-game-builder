package
{
   ...
   public class BuilderMain extends AbstractApplication
   {
      ...
      private function diagnosticInit() : void
      {
         ...
         diagnosticLog("ENGINE HARNESS v3 - original character, physics and bitmap renderer. F2 toggles log.");
      ...
      private function harnessRendered(event:Event) : void
      {
         ...
         diagnosticLog("Character frames rendered. Creating original controller and physics world.");
         harnessClip = new MovieClip();
         addChild(harnessClip);
         harnessGame = new GamePlayer(harnessClip,null);
         harnessGame.harnessStart(harnessMedia);
         stage.addEventListener(KeyboardEvent.KEY_DOWN,harnessKey);
         stage.focus = stage;
         diagnosticLog("READY: click game; arrows/A-D move, Space jumps, Down crouches. R restarts. Reach green finish zone.");
      }
      private function harnessKey(event:KeyboardEvent) : void
      {
         if(event.keyCode == 82 && harnessGame != null) harnessGame.harnessRestart();
      }
