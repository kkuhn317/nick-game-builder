package services
{
   ...
   public class ProxyManager extends EventDispatcher
   {
      ...
      private var oProxy:*;
      private var diagnosticLocalProxy:Boolean = false;
      ...
      {
         diagnosticLocalProxy = BuilderMain.instance.loaderInfo.parameters.localProxy == "true";
         if(diagnosticLocalProxy)
         {
            if(!ServiceManager.instance.fakedServicePath)
            {
               BuilderMain.instance.diagnosticLog("LOCAL PROXY ERROR: fakedService_path must be configured.");
               throw new Error("Local proxy requires fakedService_path.");
            }
            BuilderMain.instance.diagnosticLog("LOCAL PROXY: using development services at " + ServiceManager.instance.fakedServicePath + "; website callbacks disabled. Save/publish backends are not implemented.");
            this.onProxyReady(null);
            return;
         }
      ...
      internal function getServiceUrl(_sService:String) : URLRequest
      {
         if(diagnosticLocalProxy)
         {
            BuilderMain.instance.diagnosticLog("UNSUPPORTED LOCAL SERVICE: " + _sService);
            throw new Error("No local implementation for proxy service: " + _sService);
         }
      ...
      public function sendTrackingCall(_sCallback:String, _sArgs:String = null) : void
      {
         ...
         if(!diagnosticLocalProxy && ExternalInterface.available)
         {
      ...
      private function onLoadComplete(_e:Event) : void
      {
         ...
         BuilderMain.instance.diagnosticLog("ProxyModule downloaded; checking root.proxy.");
         if(!("proxy" in _e.target.content) || _e.target.content.proxy == null)
         {
            BuilderMain.instance.diagnosticLog("DEPENDENCY ERROR: ProxyModule.swf has no root.proxy object. The local file is a font-library placeholder.");
            throw new Error("Diagnostic: ProxyModule.swf must expose root.proxy; current file does not.");
         }
