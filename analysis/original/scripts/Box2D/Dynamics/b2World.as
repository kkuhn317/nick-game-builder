package Box2D.Dynamics
{
   ...
   public class b2World
   {
      ...
      public function SetBroadPhase(broadPhase:IBroadPhase) : void
      {
         ...
         for(var b:b2Body = this.m_bodyList; Boolean(b); b = b.m_next)
         {
            for(f = b.m_fixtureList; Boolean(f); f = f.m_next)
            {
      ...
      public function DestroyBody(b:b2Body) : void
      {
         ...
         while(Boolean(jn))
         {
            jn0 = jn;
            jn = jn.next;
            if(Boolean(this.m_destructionListener))
            {
      ...
      public function CreateJoint(def:b2JointDef) : b2Joint
      {
         ...
         if(Boolean(j.m_bodyA.m_jointList))
         {
         ...
         if(Boolean(j.m_bodyB.m_jointList))
         {
      ...
      public function DestroyJoint(j:b2Joint) : void
      {
         ...
         bodyA.SetAwake(true);
         bodyB.SetAwake(true);
         if(Boolean(j.m_edgeA.prev))
         {
            j.m_edgeA.prev.next = j.m_edgeA.next;
         }
         if(Boolean(j.m_edgeA.next))
         {
         ...
         j.m_edgeA.prev = null;
         j.m_edgeA.next = null;
         if(Boolean(j.m_edgeB.prev))
         {
            j.m_edgeB.prev.next = j.m_edgeB.next;
         }
         if(Boolean(j.m_edgeB.next))
         {
      ...
      public function CreateController(controller:b2Controller) : b2Controller
      {
         ...
         if(Boolean(this.m_controllerList))
         {
      ...
      {
         controller.Clear();
         if(Boolean(controller.m_next))
         {
            controller.m_next.m_prev = controller.m_prev;
         }
         if(Boolean(controller.m_prev))
         {
      ...
      {
         if(Boolean(this.m_flags & e_newFixture))
         {
      ...
      {
         for(var body:b2Body = this.m_bodyList; Boolean(body); body = body.m_next)
         {
      ...
      public function DrawDebugData() : void
      {
         ...
         if(Boolean(flags & b2DebugDraw.e_shapeBit))
         {
            for(b = this.m_bodyList; Boolean(b); b = b.m_next)
            {
               xf = b.m_xf;
               for(f = b.GetFixtureList(); Boolean(f); f = f.m_next)
               {
                  s = f.GetShape();
                  if(b.IsActive() == false)
                  {
         ...
         if(Boolean(flags & b2DebugDraw.e_jointBit))
         {
            for(j = this.m_jointList; Boolean(j); j = j.m_next)
            {
               this.DrawJoint(j);
            }
         }
         if(Boolean(flags & b2DebugDraw.e_controllerBit))
         {
            for(c = this.m_controllerList; Boolean(c); c = c.m_next)
            {
               c.Draw(this.m_debugDraw);
            }
         }
         if(Boolean(flags & b2DebugDraw.e_pairBit))
         {
            color.Set(0.3,0.9,0.9);
            for(contact = this.m_contactManager.m_contactList; Boolean(contact); contact = contact.GetNext())
            {
         ...
         if(Boolean(flags & b2DebugDraw.e_aabbBit))
         {
            bp = this.m_contactManager.m_broadPhase;
            vs = [new b2Vec2(),new b2Vec2(),new b2Vec2(),new b2Vec2()];
            for(b = this.m_bodyList; Boolean(b); b = b.GetNext())
            {
               if(b.IsActive() != false)
               {
                  for(f = b.GetFixtureList(); Boolean(f); f = f.GetNext())
                  {
         ...
         if(Boolean(flags & b2DebugDraw.e_centerOfMassBit))
         {
            for(b = this.m_bodyList; Boolean(b); b = b.m_next)
            {
               xf = s_xf;
      ...
      b2internal function Solve(step:b2TimeStep) : void
      {
         ...
         for(var controller:b2Controller = this.m_controllerList; Boolean(controller); controller = controller.m_next)
         {
         ...
         for(var c:b2Contact = this.m_contactList; Boolean(c); c = c.m_next)
         {
            c.m_flags &= ~b2Contact.e_islandFlag;
         }
         for(var j:b2Joint = this.m_jointList; Boolean(j); j = j.m_next)
         {
            j.m_islandFlag = false;
         }
         var stackSize:int = this.m_bodyCount;
         var stack:Vector.<b2Body> = this.s_stack;
         for(var seed:b2Body = this.m_bodyList; Boolean(seed); seed = seed.m_next)
         {
            if(!(Boolean(seed.m_flags & b2Body.e_islandFlag)))
            {
               if(!(seed.IsAwake() == false || seed.IsActive() == false))
               {
                  if(seed.GetType() != b2Body.b2_staticBody)
                  {
                     ...
                     while(stackCount > 0)
                     {
                        ...
                        {
                           for(ce = b.m_contactList; Boolean(ce); ce = ce.next)
                           {
                              if(!(Boolean(ce.contact.m_flags & b2Contact.e_islandFlag)))
                              {
                                 if(!(ce.contact.IsSensor() == true || ce.contact.IsEnabled() == false || ce.contact.IsTouching() == false))
                                 {
                                    ...
                                    other = ce.other;
                                    if(!(Boolean(other.m_flags & b2Body.e_islandFlag)))
                                    {
                           ...
                           for(jn = b.m_jointList; Boolean(jn); jn = jn.next)
                           {
      ...
      b2internal function SolveTOI(step:b2TimeStep) : void
      {
         ...
         for(c = this.m_contactList; Boolean(c); c = c.m_next)
         {
         ...
         while(true)
         {
            ...
            for(c = this.m_contactList; Boolean(c); c = c.m_next)
            {
               ...
               {
                  toi = 1;
                  if(Boolean(c.m_flags & b2Contact.e_toiFlag))
                  {
                     toi = c.m_toi;
                  }
                  else
            ...
            else if(minContact.IsTouching() != false)
            {
               ...
               while(queueSize > 0)
               {
                  ...
                  {
                     for(cEdge = b.m_contactList; Boolean(cEdge); cEdge = cEdge.next)
                     {
                        if(island.m_contactCount == island.m_contactCapacity)
                        {
                           break;
                        }
                        if(!(Boolean(cEdge.contact.m_flags & b2Contact.e_islandFlag)))
                        {
                     ...
                     for(jEdge = b.m_jointList; Boolean(jEdge); jEdge = jEdge.next)
                     {
