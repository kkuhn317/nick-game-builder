package Box2D.Dynamics
{
   ...
   public class b2Body
   {
      ...
      private var m_userData:IContactManaged;
      ...
      public function CreateFixture(def:b2FixtureDef) : b2Fixture
      {
         ...
         if(Boolean(this.m_flags & e_activeFlag))
         {
      ...
      public function SetPositionAndAngle(position:b2Vec2, angle:Number) : void
      {
         ...
         for(f = this.m_fixtureList; Boolean(f); f = f.m_next)
         {
      ...
      public function Split(callback:Function) : b2Body
      {
         ...
         for(var f:b2Fixture = body1.m_fixtureList; Boolean(f); )
         {
            if(Boolean(callback(f)))
            {
               next = f.m_next;
               if(Boolean(prev))
               {
      ...
      public function Merge(other:b2Body) : void
      {
         ...
         var next:b2Fixture = null;
         for(f = other.m_fixtureList; Boolean(f); )
         {
            next = f.m_next;
      ...
      public function ResetMassData() : void
      {
         ...
         for(var f:b2Fixture = this.m_fixtureList; Boolean(f); f = f.m_next)
         {
            if(f.m_density != 0)
            {
      ...
      public function SetType(type:uint) : void
      {
         ...
         this.m_torque = 0;
         for(var ce:b2ContactEdge = this.m_contactList; Boolean(ce); ce = ce.next)
         {
      ...
      public function GetUserData() : IContactManaged
      {
         return this.m_userData;
      }
      
      public function SetUserData(data:IContactManaged) : void
      {
      ...
      b2internal function ShouldCollide(other:b2Body) : Boolean
      {
         ...
         for(var jn:b2JointEdge = this.m_jointList; Boolean(jn); jn = jn.next)
         {
            if(jn.other == other)
            {
